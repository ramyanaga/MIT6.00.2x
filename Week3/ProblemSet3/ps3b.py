...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for timeStep in range(300):
            patientTimeSteps = []
            patientTimeSteps.append(float(patient.update()))
            virusPopSizes.append(patientTimeSteps)
    ...
    ys = []
    for i in range(300):
        ys.append(float(sum(virusPopSizes[i]))/float(len(virusPopSizes[i])))
...
#simulationWithoutDrug(100,1000,0.1,.05,300)
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            if self.isResistantTo(drug):
                resistanceCount+=1
        if resistanceCount==len(activeDrugs) and random.random() < self.maxBirthProb * (1 - popDensity):
            childResistances = {}
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.viruses:
            ...
            if fullResistance:
                resistantToAllCount+=1
        return resistantToAllCount
    ...
    def update(self):
        ...
        for virus in self.viruses[:]:
            try: 
                reproducedVirus = virus.reproduce(popDensity,self.drugs)
                if reproducedVirus:
                    self.viruses.append(reproducedVirus)
            except NoChildException:
                continue
        return self.getTotalPop()
        """
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    numTrials: number of simulation runs to execute (an integer)
    """
    totalVirusesPop, resistantVirusPop = [], []
    for trial in range(numTrials):
        totalVirusesPop.append([])
        resistantVirusPop.append([])
    timeSteps = 300
    treatmentStep = 150
    for trial in range(numTrials):
        ...
        for step in range(timeSteps):
            ...
            patient.update()
            totalVirusesPop[step].append(patient.getTotalPop())
            resistantVirusPop.append(patient.getResistPop(["guttagonol"]))
    totalVirusesPopAverage = [sum(v)/float(len(v)) for v in totalVirusesPop]
    resistantVirusPopAverage = [sum(v)/float(len(v)) for v in resistantVirusPop]
    pylab.plot(totalVirusesPop, label = "Total Virus Population")
    ...
    for trial in range(numTrials):
        viruses = []
        for i in range(100):
            viruses.append(ResistantVirus(maxBirthProb,clearProb,resistances,clearProb))
        patient = TreatedPatient(viruses,maxPop)
    for i in range(300):
        patient.update()
        totalVirusesPop.append(patient.getTotalPop())
        resistantVirusPop.append(patient.getResistPop(['guttagonol']))
        if i == 150:
            patient.addPrescription('guttagonol')
    for i in range(300):
        totalVirusesPop[i] = float(totalVirusesPop[i])/300
        resistantVirusPop[i]=float(resistantVirusPop[i])/300

    pylab.plot(totalVirusesPop,label='Total Virus Pop')
    pylab.plot(resistantVirusPop,label='Resistant Virus Pop')
